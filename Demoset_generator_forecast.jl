...
# ╔═╡ d9223295-f825-46b0-be5c-bc78ecd4fe98
md" ##### 🔽 Define time points"

# ╔═╡ 652745a6-6a26-44ca-ad04-fe8dfda10f96
begin
	@bind values confirm(
		PlutoUI.combine() do Child
		md"""
		Choose start date: $(Child(DateField(default= Date("2018-01-01"))))
		$br
		Choose end date: $(Child(DateField(default= Date("2020-01-03"))))
		$br
		Choose time unit: $(Child(Select(["Months", "Days"])))
		$br
		Choose step size:  $(Child(NumberField(1:10)))  
		$br	 
		""" 
		end)
end

# ╔═╡ 809291d9-b6f5-4f29-8a80-837bd1f72407
begin
	(start_date, end_date, unit, step_size) = values
	@assert end_date > start_date 
	start_date, end_date = Dates.Date(start_date), Dates.Date(end_date)
end;

# ╔═╡ 68659ef4-60f6-43cf-b2d2-e7d64839a294
begin
	if  unit == "Months"
		time_points = collect(Dates.Date(start_date):Month(1):Dates.Date(end_date));
	elseif unit == "Days"
		time_points = collect(Dates.Date(start_date):Day(1):Dates.Date(end_date));
	end
	time_points
end

# ╔═╡ 13258a75-9f44-4eb0-acc7-5130fe09ccc6
n_samples = length(time_points)

# ╔═╡ 7ed7b8bf-ead2-46f2-8708-9aba3c37e5e3
md" ##### 🔽 Define variables"

# ╔═╡ 0961c99e-6ca6-4431-9d43-517e8cd52607
...
# ╔═╡ 206d16a8-4447-4d87-bace-5d5199af6536
...
md""" B: $(@bind B Slider(10:500))"""

# ╔═╡ 4f885af1-546a-47ec-897e-1d4683986ef9
plot(sales_series.(1:n_samples, A, B), title="Sales time series", label="", xlabel="$unit", ylabel="$target")
...
begin
		...
		for product in Product_groupes
			append!(on_promotion_vec, rand(On_promotion, n_samples));
		end
		on_promotion_vec = repeat(on_promotion_vec, length(Clients));
end;
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "e5999c26ef83cacb2e0615f84158be07d18ed180"
